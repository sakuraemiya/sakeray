...
<mapper namespace="${package.dao}.${daoClass}">
    ...
    <select id="selectPage" resultMap="BaseResultMap" parameterType="${package.request}.${requestSearchClass}">
        ...
        order by ${primaryId} desc
        <if test="pageEnable">
	        limit ${r"#{" + "start,jdbcType=INTEGER" + r"}"},${r"#{" + "limit,jdbcType=INTEGER" + r"}"}
        </if>
        </#if>
        <#if dbType == 'ORACLE'>
        ...
        from (
            select A.*, ROWNUM RN
            from (SELECT * FROM ${tableName} <include refid="pageWhereSql"/>) A
            WHERE ROWNUM &lt;= ${r"#{" + "end,jdbcType=INTEGER" + r"}"}
        ) WHERE RN &gt;= ${r"#{" + "start,jdbcType=INTEGER" + r"}"}
        </#if>
    </select>
