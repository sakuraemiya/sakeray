...
+ redis主从复制的过程
    ...
    + ![Redis主从复制](./image/Redis主从复制.PNG "Redis主从复制")
...
20. ACID是靠什么来保证的
    ...
    + 一致性是由其它三大特征保证
    + 隔离性是由MVCC来保证
    + 持久性是由内存加redo log日志保证，mysql修改数据会在内存和redo log记录，宕机后从redo log恢复。InnoDB redo log写盘，InnoDB事务进入prepare状态，如果prepare成功，binlog在写盘，再继续将事务日志持久化到binlog，如果持久化成功，那么InnoDB事务进入commit状态，在redo log里面写一个commit记录，redo log刷盘在空闲期进行
21. MVCC解决的问题是什么
...
24. MVCC实现机制
    + 聚簇索引记录中有两个隐藏列，通过这两个列形成了版本链
        + trx_id:用来存储每次对某个记录进行修改的事务id
        + roll_pointer:每次对某条记录修改的时候，都会把老版本写入undo日志中，这个roll_pointer就是存了一个指针，指向这条记录的上一个版本的位置
    + ReadView：开始事务时创建readView，readView维护了当前活动的事务id，即未提交的事务id，生成一个数组，访问数据获取数据中的事务id，对比readView如果比readView小，则可以访问，反之不能，并通过roll_pointer找到一个比readView小的事务访问其数据
    + 读已提交在每次查询时候都会生成一个readView，无法防止不可重复读；而可重复读只在第一次读时生成一个readView，后面都是复用这个readView
